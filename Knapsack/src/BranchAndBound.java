...
class Item {
	...
	// Stores the weight of items (pi)
	float weight;

	// Stores the values of items (ci)
	int value;

	// Stores the index of items (i = 0:n)
	int idx;
	public Item() {}
...
class Node {
	// Borne superior: Best case
	// (Fractional Knapsack)
	float ub; 

	// Borne inferior: Worst case
	// (1 if said item taken; 0 if not)
	float lb; 
	...
	// Total Weight: Stores the sum of the weights of items included
	float tw;
...
// Comparator to sort based on lb 
...
// Comparator to sort ratio ci/pi
...
class knapsack {
	...
	// Calculate lb
	// (fractional part of items not included)
	...
	public static void solve(Item arr[])
	{
		...
		// of all the paths that reached final level
		...
		while (!pq.isEmpty()) {
			...
			// Hence, cp, cw will obtain the value of that parent
			...
			if (current.tw + arr[current.level].weight
				...
				// c and lb calculated + current item included
				left.ub = upperBound(
					current.tv
						- arr[level].value,
					current.tw
		...
		System.out.println("Les objets choisi sont :");
		for (int i = 0; i < size; i++) {
			if (finalPath[i])
				System.out.print("1 ");
			else
		...
		System.out.println("\nMaximization de l'utilité total maxZ"
						+ " = " + (-finalLB));
	}
